...
export default function StaffAttendance() {
  // Initial Mock Data for Staff & Shifts
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-[1600px] mx-auto font-sans bg-slate-50/50 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 pb-5">
        <div>
          <h2 className="text-xl sm:text-2xl font-black text-slate-900">
            Staff Duty Roster & Attendance History
          </h2>
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
            Monitor doctor and nurse arrivals, departure timestamps, duty hours, and shift management.
          </p>
        </div>
        <div className="flex items-center gap-2 bg-white border border-slate-200 px-3 py-2 rounded-xl text-xs font-bold text-slate-700 shadow-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-4 bg-white border border-slate-200 rounded-2xl shadow-sm space-y-2">
          <div className="flex justify-between items-center text-slate-500">
            <span className="text-[11px] font-bold uppercase tracking-wider">Early Exits</span>
            <LogOut className="w-5 h-5 text-red-500" />
          </div>
          <span className="text-2xl font-black text-slate-900">
            {staffMembers.filter(s => s.timingStatus.includes('Early')).length}
          </span>
        </div>
      </div>
          ...
          <table className="w-full text-left text-xs text-slate-600">
            <thead className="bg-slate-50 text-slate-500 uppercase tracking-wider text-[10px] font-bold border-b border-slate-100">
              <tr>
                ...
                <th className="p-3.5">Assigned Shift Timing</th>
                <th className="p-3.5">Clock-In (Login)</th>
                <th className="p-3.5">Clock-Out (Logout)</th>
                <th className="p-3.5">Total Duty Hours</th>
                ...
                <tr key={staff.id} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="p-3.5 text-right">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 text-xs bg-slate-100 hover:bg-slate-200 text-slate-700 px-3 py-1.5 rounded-lg border border-slate-300 font-bold transition-all"
                    >
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white border border-slate-200 rounded-2xl p-6 w-full max-w-md shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              <h3 className="text-base font-bold text-slate-900">Change Assigned Shift</h3>
